...
def register():
    ...
    if request.method == "POST":
        if User.query.filter_by(email=form.email.data).first() != None:
            flash("You have already used this email, please log in with that email")
...
def login():
    ...
    if request.method == "POST":
        ...
        if user == None:
            flash("We couldn't find such a email in our database")
            return redirect("login")
        if check_password_hash(user.password, form.password.data):
            login_user(user)
            return redirect(url_for("get_all_posts"))
        flash("Wrong Password, Please try again later")
...
def show_post(post_id):
    ...
    if request.method == "POST":
        if not current_user.is_authenticated:
            ...
            return redirect(url_for("login"))
        new_comment = Comment(text=comment.comment.data, author_id=current_user.id, post_id=post_id)
        db.session.add(new_comment)
        db.session.commit()
    return render_template("post.html", post=requested_post, form=form, comments=comms)
